...
export default function AdminSchedules() {
  ...
  const [showAutoDistributeDialog, setShowAutoDistributeDialog] =
    useState(false);
  ...
  const { data: ministryMembers, isLoading: loadingMembers } =
    useMinistryMembers(selectedMinistryId ? Number(selectedMinistryId) : 0);
  ...
  const publishSchedule = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Os voluntários receberam notificações.",
      });
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground transition-all">
      ...
      <main className="flex-1 md:ml-64 p-6 md:p-10 space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <Button
            ...
            size="lg"
            className="gap-2 rounded-2xl h-14 px-6 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 shadow-lg"
          >
            <Sparkles className="h-5 w-5" />
            <span className="font-bold">Distribuição Automática</span>
          </Button>
        </header>
            ...
            <Card className="p-6 border-none rounded-[2.5rem] bg-card/50 backdrop-blur-sm shadow-xl ring-1 ring-white/10">
                ...
                <div className="space-y-2">
                  ...
                  <Select onValueChange={handleContextChange}>
                    ...
                    <SelectContent className="rounded-2xl border-none shadow-2xl max-h-[300px]">
                      {/* SERVICES */}
                      <div className="px-2 py-2 text-[10px] font-black uppercase text-muted-foreground">
                        Cultos Recorrentes
                      </div>
                      {services?.map((s) => (
                        <SelectItem
                          key={`SERVICE:${s.id}`}
                          value={`SERVICE:${s.id}`}
                          className="py-3 font-bold border-b border-border/50"
                        >
                          <span className="flex items-center gap-2">
                            <Clock className="w-4 h-4 text-primary" />
                            {s.name}{" "}
                            <span className="opacity-50 text-[10px] font-medium uppercase">
                              ({s.time})
                            </span>
                          </span>
                        </SelectItem>
                      ...
                      {/* EVENTS */}
                      <div className="px-2 py-2 text-[10px] font-black uppercase text-muted-foreground mt-2">
                        Eventos Especiais
                      </div>
                      {events?.map((e) => (
                        <SelectItem
                  ...
                  <Popover>
                    ...
                    >
                      <Calendar
                        ...
                        } // If Event, lock date (optional UX choice)
                        initialFocus
              ...
              <Tabs defaultValue="available" className="w-full">
                <TabsList className="grid grid-cols-2 bg-muted/50 p-1 rounded-2xl h-14 mb-4">
                  <TabsTrigger
                    value="available"
                    className="rounded-xl font-black data-[state=active]:bg-primary data-[state=active]:text-white"
                  >
                    DISPONÍVEIS ({availableCandidates.length})
                  </TabsTrigger>
                  <TabsTrigger
                    value="unavailable"
                    className="rounded-xl font-black data-[state=active]:bg-muted-foreground/20"
                  >
              ...
              <div className="flex justify-between items-center mb-8 relative z-10">
                ...
                {draftTeam.length > 0 && (
                  <Button
                    ...
                    className="text-destructive font-bold uppercase text-xs hover:bg-destructive/10"
                  >
                    Limpar
                  </Button>
                )}
      ...
      <AutoDistributeDialog
        open={showAutoDistributeDialog}
        onOpenChange={setShowAutoDistributeDialog}
